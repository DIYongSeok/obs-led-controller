  ...
  BROADCAST.call('GetInputList', { inputKind: 'ffmpeg_source' })
    .then(async (data) => {
      const result = await Promise.all(
        data.inputs.map(async (val) => {
          const volume = (
            await BROADCAST.call('GetInputVolume', {
              inputName: val.inputName as string,
            })
          ).inputVolumeDb;
...
router.post('/set', (req, res, next) => {
  ...
  BROADCAST.call('SetInputVolume', {
    inputName,
    inputVolumeDb: parseFloat(volume),
  });
  res.end();
});
